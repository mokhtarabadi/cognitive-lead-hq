...
status: active
tags: []
updated_at: '2026-08-19T05:09:27.234316+00:00'
---
...
| Skills (29) | `~/.config/opencode/skills/<name>/SKILL.md` | `~/.agents/skills/<name>/SKILL.md` |
...
4. **Smoke-test** servers launch (`uv run <server>.py --help`) and run the repo test suite (45 passed expected):
   ```bash
...
- Last run: 2026-08-19 — only drift was `mcp-lint-server/server.py` (old Aug 13 copy missing `lint_system_prompt_sync`) and `system-prompt.md` (v8.4.5 → v8.4.6). All 29 skills ×2, all 4 agents, context+memory servers, shell-strategy, and `~/.AGENTS.md` were already identical.
