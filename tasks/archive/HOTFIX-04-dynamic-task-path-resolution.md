# Task HOTFIX-04: Dynamic Task Path Resolution

**File:** `tasks/archive/HOTFIX-04-dynamic-task-path-resolution.md`
**Source:** orchestrator
**Type:** improvement
**Status:** superseded
...
- [ ] Read AGENTS.md, docs/conventions.md, daemon.py
- [ ] Step 1 — add `resolve_actual_task_path(task_file, repo_root)` helper in daemon.py (exact spec snippet)
- [ ] Step 2 — integrate resolver into `trigger_task` (path existence + DB path sync when moved)
- [ ] Step 3 — integrate resolver at the start of `process_task`/`_process_task`
...
- **Actual result:** 247 passed, 0 failed in 13.65s. Functional resolver harness verified 7 semantics: identity (absolute), identity (relative-exists), moved-found (backlog→in-progress rel path), missing-everywhere (unchanged), search-order in-progress wins, qa fallback, completed last-resort, identity-wins-over-duplicates.
- **Exit code:** 0
...
- **[2026-08-31] [D1] [LITE]:** Orchestrator tagged this task `lite_mode` although it modifies one source file plus requires a task-file recreation (absent from all Kanban dirs again).
  - **Rationale:** Steps were fully specified with exact code snippets; single source file (daemon.py); zero architectural ambiguity.
...
- **[2026-08-31] [D2] [EXECUTOR-DETECTED]:** Task file was absent from all Kanban directories despite Orchestrator metadata; recreated at the exact Orchestrator-specified path (same pattern as HOTFIX-01 D4 / HOTFIX-02 D2 / HOTFIX-03 D2).
...
- **[2026-08-31] [D3] [EXECUTION-DETECTED]:** During placement of the resolver, an earlier edit accidentally deleted `extract_task_diff`'s docstring; restored it verbatim and inserted `resolve_actual_task_path` above it.
  - **Rationale:** Caught during the read-back verification step (Plan→Execute→Observe); the file never left the working tree in a broken state and syntax was re-verified.
  - **Alternatives considered:** Leaving the docstring-less state — rejected: violates documentation hygiene and the file's established style.
  - **Impact:** `extract_task_diff` byte-identical to pre-edit (verified by full-suite green).
- **[2026-08-31] [D4] [EXECUTION-DETECTED]:** Integration scope — `process_task` resolves + re-syncs the DB (best-effort) and passes the resolved string onward; `_process_task` resolves AGAIN for defense-in-depth (idempotent, cheap).
  - **Rationale:** The spec asks for resolution at the beginning of both. `process_task` is the public seam (tests call it with absolute existing paths → resolver no-ops); `_process_task` is called directly by test_le0_fixes/test_specs/test_contracts with absolute paths → no-op. The double resolve guarantees correctness for any caller.
  - **Alternatives considered:** Resolving only in `_process_task` — rejected: `trigger_task`'s DB sync and the `process_task` guard would diverge from the resolved path.
  - **Impact:** No test-visible behavior change for recorded paths; move-tolerance is additive.
- **[2026-08-31] [D5] [EXECUTION-DETECTED]:** Removed the now-dead local `from pathlib import Path` in `trigger_task` after the resolver took over Path construction.
  - **Rationale:** Dead code introduced in the HOTFIX-01 era; the resolver returns a Path directly.
  - **Alternatives considered:** Leaving it — harmless but misleading.
  - **Impact:** None functional; cleaner diff.
...
> **Superseded:** This task was bundled into META task `149-hotfix-bundle-telegram-gateway-telemetry-path-reasoning-concurrency` and archived on 2026-09-01. See `tasks/qa/149-hotfix-bundle-telegram-gateway-telemetry-path-reasoning-concurrency.md` for the unified execution. History preserved via `git log --follow -- tasks/archive/HOTFIX-04-dynamic-task-path-resolution.md`.

## Execution Log & Reasoning

**2026-08-31 — HOTFIX-04 applied (Plan→Execute→Observe):**

1. **Verify-before-apply:** Confirmed callers/tests that touch the modified seams: `process_task` (public seam, called by tests with absolute existing paths), `_process_task` (called directly by test_le0_fixes/test_specs/test_contracts with absolute existing paths), `trigger_task` (no direct test; gateway/CLI path). The resolver is a pure no-op for existing recorded paths — zero test-visibility change for the common case.
2. **Step 1 — `resolve_actual_task_path`:** added module-level in daemon.py verbatim per spec (identity when recorded path exists; else search `in-progress → qa → backlog → completed`; returns recorded path unchanged if not found). Placement: above `extract_task_diff` (utility area). An intermediary edit glitch deleted `extract_task_diff`'s docstring — restored verbatim (D3).
3. **Step 2 — `trigger_task`:** replaced the direct `Path(task_file)` existence check with the resolver; added best-effort DB re-sync (`UPDATE tasks SET task_file=...`) when `actual_rel_path != task_file`, then reassigns `task_file` so `process_task` launches with the resolved path. Removed the now-dead local `from pathlib import Path` import (D5).
4. **Step 3 — `process_task`:** resolves, re-syncs the DB best-effort, forwards `resolved_task_file` into `_process_task`; missing-file semantics unchanged (crash path still handled by the existing exception guard). `_process_task` additionally resolves at its head (D4 defense-in-depth) so `task_path` always points at the real on-disk file for the fresh read.
5. **Observe:** py_compile OK; resolver harness proved all 7 semantics (identity absolute/relative, moved-found rel path, missing unchanged, in-progress/qa/completed search order, identity-wins-over-duplicates when the recorded path exists); full suite **247 passed, 0 failed** (13.65s).
6. **Scope guard:** single source file changed for this hotfix: `loop-engine/daemon.py` — plus task file + CHANGELOG. Prior hotfix staged artifacts remain in the index (their QA lifecycle); out-of-band `loop-engine.jsonc`/models.py/verifier.py/blast_radius.py untouched.
