...
2. **`skill-templates/perplexity-research/SKILL.md`** — The companion Agent Skill that teaches OpenCode *when* to trigger this loop and *how* to formulate the exact message block for the Manager. The halt-and-wait pattern prevents the agent from proceeding with incomplete or hallucinated information.
