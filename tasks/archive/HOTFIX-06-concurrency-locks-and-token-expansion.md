# Task HOTFIX-06: Concurrency Locks and Token Expansion

**File:** `tasks/archive/HOTFIX-06-concurrency-locks-and-token-expansion.md`
**Source:** orchestrator
**Type:** improvement
**Status:** superseded
...
- [ ] Read AGENTS.md, docs/conventions.md, daemon.py, router.py, gateway.py, state.py
- [ ] Step 1 — daemon.py: `_in_flight_tasks` set in `__init__`; duplicate-trigger guard in `trigger_task`; lock wrap in `process_task` via registered daemon instance
- [ ] Step 2 — router.py: `max_tokens` 4096 → 8192 in `call_llm`; anti-runaway plan prompt in `route_plan` (keeping brainstorm `extra_context` injection)
- [ ] Step 3 — gateway.py: `_processed_callback_ids` set; instant `cq.answer()` before processing; dedup by query id; `_handle_text_command` wrapped so network hiccups don't kill the poller
...
- **Actual result:** 247 passed, 0 failed in 13.24s. Functional harnesses verified: daemon lock (first call runs + discards id, pre-added in-flight id early-returns without running, trigger_task guard fires before resolution), gateway poller (duplicate callback query id answered exactly once, `trigger_task:5` triggered once, `/status` + unknown command contained, instant ack before processing), route_plan prompt (anti-runaway rules + brainstorm `extra_context` retained) and `max_tokens: 8192`.
- **Exit code:** 0
...
- **[2026-08-31] [D1] [LITE]:** Orchestrator tagged this task `lite_mode` although it touches 3 source files (daemon.py, router.py, gateway.py) plus task recreation.
  - **Rationale:** All 3 steps were fully specified (exact code for the lock guard, exact prompt, exact poller changes); localized hardening edits with zero architectural ambiguity.
...
- **[2026-08-31] [D2] [EXECUTOR-DETECTED]:** Task file was absent from all Kanban directories despite Orchestrator metadata; recreated at the exact Orchestrator-specified path (same pattern as HOTFIX-01..05).
...
- **[2026-08-31] [D3] [EXECUTION-DETECTED]:** `process_task` is a module-level function without a daemon parameter; the in-flight lock accesses the registry via `gateway._daemon` (`getattr(gateway, "_daemon", None)`), with a graceful no-op when no daemon is registered (legacy/test callers).
  - **Rationale:** `gateway.set_daemon()` is the established wiring seam (set in main and in the smoke workspace), so the daemon is reachable from every process_task call site (`trigger_task`, `boot_scan`, watcher, `--run`). The spec's `daemon_instance` reference is satisfied by this accessor.
  - **Alternatives considered:** A module-level global set — rejected (global mutable state violates the daemon encapsulation and would leak across daemon instances in tests); threading the daemon through process_task's signature — rejected (changes the public signature used by tests).
  - **Impact:** Lock is active for all real daemon paths; direct `_process_task` callers (test_le0_fixes/test_specs/test_contracts) are intentionally unaffected.
- **[2026-08-31] [D4] [EXECUTION-DETECTED]:** The poller now answers each callback query ONCE, immediately and WITHOUT the ack toast text; the old `answer_callback_query(cq.id, text=ack)` after `handle_callback` was removed.
  - **Rationale:** Telegram rejects a second answer to the same query ("query is too old / already answered"); instant empty ack is the spec's explicit first-line requirement and prevents expiry while the dedup set still records the query.
  - **Alternatives considered:** Computing `handle_callback`'s ack first then answering with text — preserves the toast but violates the spec's "answer in the first line before processing" directive.
  - **Impact:** Button press UX loses the ephemeral toast; nothing functional depends on it (no test asserts it; the trigger/approve effects still occur server-side).
- **[2026-08-31] [D5] [EXECUTION-DETECTED]:** The spec's new `route_plan` prompt snippet omits brainstorming `extra_context`; I retained the `if extra_context:` injection after the spec'd base prompt.
  - **Rationale:** The brainstorm stage (Phase 1.5) is a live feature; silently dropping its context from the plan prompt would regress pipeline behavior (brainstorm-approved tasks would plan without the session output).
  - **Alternatives considered:** Replacing the prompt verbatim without the injection — rejected as a functional regression beyond the hotfix's intent.
  - **Impact:** Spec'd directives preserved verbatim; brainstorm context still appended when present.
...
> **Superseded:** This task was bundled into META task `149-hotfix-bundle-telegram-gateway-telemetry-path-reasoning-concurrency` and archived on 2026-09-01. See `tasks/qa/149-hotfix-bundle-telegram-gateway-telemetry-path-reasoning-concurrency.md` for the unified execution. History preserved via `git log --follow -- tasks/archive/HOTFIX-06-concurrency-locks-and-token-expansion.md`.

## Execution Log & Reasoning

**2026-08-31 — HOTFIX-06 applied (Plan→Execute→Observe):**

1. **Verify-before-apply:** Confirmed test couplings: `process_task` is the public seam (smoke suite runs real executions — the lock must not deadlock or crash them; direct `_process_task` callers in test_le0_fixes/test_specs/test_contracts bypass the lock entirely); `trigger_task` has no direct test; the poller is not unit-tested (test_audit_fixes tests `handle_callback` return values only — unaffected since the callback branch now ignores the ack string); `route_plan`'s user string only asserted for task-content presence (test_route_plan).
2. **Step 1 — daemon.py:** `__init__` gained `self._in_flight_tasks: set[int] = set()`. `trigger_task` early-returns with the spec'd log line when the task id is in flight. `process_task` resolves the daemon via `gateway._daemon`, re-checks the set (early return + log), `add`s before execution, and `discard`s in `finally` (D3). Note: an intermediate edit left a dangling outer `try:` — caught during read-back, removed, structure re-verified.
3. **Step 2 — router.py:** `max_tokens` 4096 → 8192 in `call_llm`. `route_plan` user prompt replaced with the spec'd anti-runaway text; `extra_context` (brainstorm) injection retained (D5). Existing `<deliverable>` system block retained (HOTFIX-02) — complements the new rules.
4. **Step 3 — gateway.py:** `__init__` gained `self._processed_callback_ids: set[str]`. `_poll_loop` callback branch: dedup by query id (skip duplicates), `await cq.answer()` in try/except FIRST, mark processed, then `handle_callback` (ack toast dropped — D4). Text-command branch wrapped in try/except so handler network failures never kill the poller loop.
5. **Observe:** py_compile OK; daemon harness proved lock semantics (first-call runs + discards; pre-added id early-returns without running; trigger_task guard fires before any resolution); gateway poller harness proved dedup (query id answered once, trigger fired once, unknown and `/status` commands contained); router harness proved the prompt (rules present, brainstorm context retained) and `max_tokens: 8192`. Full suite **247 passed, 0 failed** (13.24s).
6. **Scope guard:** files changed: `loop-engine/daemon.py`, `loop-engine/router.py`, `loop-engine/gateway.py`, task file, CHANGELOG. Out-of-band files untouched; prior hotfix staged artifacts remain in the index under the accumulated-staging model.
