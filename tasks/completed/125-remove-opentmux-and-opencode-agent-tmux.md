...
index de74116..cbae9e1 100644
--- a/LLM.txt
+++ b/LLM.txt
@@ -13,7 +13,7 @@ You MUST execute the following steps in order. Do NOT skip any step.
...
@@ -132,22 +132,6 @@ chmod +x scripts/bundle-tasks.py
...
-### 6.2. Install opentmux Globally (Tmux Integration)
-
-[opentmux](https://github.com/AnganSamadder/opentmux) provides real-time tmux panes for viewing OpenCode agent execution. It wraps `opencode` with automatic tmux pane spawning, output streaming, and terminal workspace management.
-
-```bash
...
-```
-
-**Usage:** Run `opentmux` instead of `opencode` to start with tmux integration. All standard OpenCode flags work (e.g., `opentmux --agent cognitive-executor`).
-
 ---
 
 ## 6.5. Install Custom Agents Globally
@@ -324,7 +308,6 @@ After completing all steps, verify:
 - [ ] `node` is installed (`node --version`)
