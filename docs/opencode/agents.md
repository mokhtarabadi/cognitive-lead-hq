...
Allows you to control which tools are available in this agent. You can enable or disable specific tools by setting them to true or false. In an agent’s tools config, true is equivalent to {"_": "allow"} permission and false is equivalent to {"_": "deny"} permission.
